$.getJSON("./data-normandie-hr.geojson", function(data) {
    ...
    function letsStart() {
        ...
        for (i in dataEQ)
            if (document.location.href.match(/tousProjets/g) || document.location.href.match(/adminView/g) || dataEQ[i].properties.Aetat !== "abandonn\u00e9") {
                ...
                if (PinCat[dataEQ[i].properties.Lcat] == undefined) iconeCat = PinCat["la9-autre"];
                else iconeCat = PinCat[dataEQ[i].properties.Lcat];
                if (PinAv[dataEQ[i].properties.Aetat] == undefined) iconeAv = PinAv["av0-int"];
